...
class InvoiceMailController extends Controller
{
    ...
    public function store($data)
    {
        $config = array(
            ...
            'port' => '587',
            'username' => 'MS_acNy0k@ifca.co.id',
            'password' => 'izh9c0lJ58KkBftD',
            'encryption' => 'tls',
            'from' => array('address' => 'system.software@ifca.co.id', 'name' => 'IFCA Software'),
        );
    ...
    public function storeWA($data)
    {
        ...
        if (!empty ($dt)) {
            for ($i = 0; $i < count($dt); $i++) {
                $month = Carbon::createFromFormat('Y-m-d H:i:s.u', $dt[$i]['doc_date'])->format('F');

                $response = Http::post(
                    // 'http://whatsapi.ifca.co.id/api/sendwa-demo/3/' . $dt[$i]['debtor_name'] . '/' . $month . '/' . $dt[$i]['filenames'] . '/08112777873'
                    'http://whatsapi.ifca.co.id/api/sendwa-demo/3/' . $dt[$i]['debtor_name'] . '/' . $month . '/' . $dt[$i]['filenames'] . '/08112777873'
                );
    ...
    public function storeJoin($data)
    {
        $dt = $data['models'];
            ...
            for ($i = 0; $i < count($dt); $i++) {
                ...
                if ($statusCode == 200) {
                    ...
                    if ($headers && strpos($headers[0], '200 OK') !== false) {
                        ...
                        $callback = array(
                            "Error" => false,
                            "Pesan" => "Invoice send successfully"
                        );
                    } else {
                        // File does not exist."
                        $callback = array(
                            "Error" => true,
                            "Pesan" => "Unable to process, because the file does not exist"
                        );
                    }
                } else {
                    $callback = array(
    ...
    public function storeMultiPlatform(Request $request)
    {
        ...
        if (in_array('email', $typesToSend) && in_array('whatsapp', $typesToSend)) {
            $res = $this->storeJoin($data);

            return response()->json($res);
        } else if (in_array('email', $typesToSend)) {
            $res = $this->store($data);
            ...
            $response = array(
                "Error" => true,
                "Pesan" => "Cannot be processed, because you have not selected the type of delivery."
            );
